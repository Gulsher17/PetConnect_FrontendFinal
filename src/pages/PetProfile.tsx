      ...
      <main className="max-w-6xl mx-auto w-full p-6 space-y-8">
        ...
        <div className="flex items-start justify-between gap-6">
          ...
          <div className="flex gap-3">
            ...
            <Link to={`/adopt/${pet?._id || pet?.id || ""}`} className="pc-btn pc-btn-primary">
              Start Process
            </Link>
          </div>
        </div>
          ...
          <aside className="pc-card p-5">
            ...
            <ul className="space-y-2 text-sm text-gray-700">
              ...
              {pet?.spayedNeutered !== undefined && (
                <li><b>Spayed/Neutered:</b> {pet.spayedNeutered ? "Yes" : "No"}</li>
              )}
              {pet?.microchipped !== undefined && (
                <li><b>Microchipped:</b> {pet.microchipped ? "Yes" : "No"}</li>
              )}
            </ul>
          </aside>
        </section>

        {/* Poster (basic; no health/behavior per your instruction) */}
        <section className="pc-card p-5">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold">
              {(pet?.organization?.name || pet?.owner?.name || "P")[0]}
            </div>
            <div>
              <p className="font-medium">
                {pet?.organization?.name || pet?.owner?.name || postedBy}
              </p>
